...
export const createCheckoutSessionSubscription = async (
  ...
  try {
    ...
    const subscription = await Subscriptions.create({
      ...
      stripeCustomerId,
      priceId,
      plan: priceDetails?.recurring?.interval?.toUpperCase() || 'MONTHLY',
      planType: 'recurring',
      interval: priceDetails?.recurring?.interval || 'month',
      status: 'pending',
    });
    ...
    // Update session metadata with subscription and transaction IDs
...
export const createCheckoutSessionOneTime = async (payload: createPlanType) => {
  try {
    ...
    // Update session metadata with transaction ID
    await stripe.checkout.sessions.update(session.id, {
      metadata: {
        ...session.metadata,
        transactionId: transaction._id.toString(),
...
export const upgradeSubscription = async (payload: upgradeSubscriptionType) => {
  try {
    ...
    // Update local subscription record with new price
    await Subscriptions.findOneAndUpdate(
      { stripeSubscriptionId: userSubscription.stripeSubscriptionId },
      {
        $set: {
          priceId: priceId,
          plan:
            newPriceDetails?.recurring?.interval?.toUpperCase() ||
            userSubscription.plan,
          interval:
            newPriceDetails?.recurring?.interval || userSubscription.interval,
        },
      }
    );
    ...
    await Transactions.create({
      ...
      type: 'subscription',
      status: 'succeeded',
      amount: newPriceDetails?.unit_amount || 0,
